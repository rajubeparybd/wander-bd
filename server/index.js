...
async function run() {
    try {
        ...
        const tourGuidesCollection = await db.collection("tourGuides");
        ...
        //get stories added by logged in user
        app.get("/stories", async (req, res) => {
            const email = req.query.email;
            if (!email) return res.status(400).send({ error: "Missing email query param" });

            const stories = await storiesCollection.find({ authorEmail: email }).toArray();
            res.send(stories);
        });
